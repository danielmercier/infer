...
type t = Clang | Java | Ada [@@deriving compare]

let equal = [%compare.equal: t]

let language_to_string = [(Clang, "C/C++/ObjC"); (Java, "Java"); (Ada, "Ada")]

let to_string lang = List.Assoc.find_exn language_to_string ~equal lang

let to_explicit_string = function Clang -> "Clang" | Java -> "Java" | Ada -> "Ada"
